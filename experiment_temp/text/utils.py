        ...
        for i in range(mat.shape[0]):
            ...
            # with separators
                ...
                for word in words:
                    ...
                    t = ctcBeamSearch(matrix, self.character, self.ignore_idx, None, beamWidth=beamWidth,
                                      dict_list=dict_list)
                    string += t
            texts.append(string)
        return texts
            ...
            for box in boxes:
                if len(new_box) == 0:
                    ...
                    if (abs(np.mean(b_height) - box[5]) < height_ths * np.mean(b_height)) and (
                            abs(box[0] - x_max) < width_ths * (box[3] - box[2])):  # merge boxes
...
def set_result_with_confidence(result_list, origin_len):
    ...
    k = 0
    result_to_split = [[] for i in range(
        set_len)]  # list having the same length of the rotation_info list : each element is a list of rotated images in the same direction
